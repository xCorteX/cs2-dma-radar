...
interface VmmNative extends Library {
	...
	boolean VMMDLL_MemReadEx(Pointer hVMM, int dwPID, long qwA, byte[] pb, int cb, IntByReference pcbReadOpt,
			int flags);
	...
	class VMMDLL_MAP_PHYSMEM extends Structure {
		...
		VMMDLL_MAP_PHYSMEM(Pointer p) {
			...
			read();
			if (dwVersion != VmmNative.VMMDLL_MAP_PHYSMEM_VERSION) {
	...
	@Structure.FieldOrder({ "dwPID", "dwState", "_FutureUse3", "AF", "Src", "Dst", "vaObj", "ftTime", "dwPoolTag",
			"_FutureUse4", "uszText", "_FutureUse2" })
	...
	class VMMDLL_MAP_NET extends Structure {
		...
		VMMDLL_MAP_NET(Pointer p) {
			...
			read();
			if (dwVersion != VmmNative.VMMDLL_MAP_NET_VERSION) {
				throw new VmmException("Bad Version");
			}
	...
	class VMMDLL_MAP_USER extends Structure {
		...
		VMMDLL_MAP_USER(Pointer p) {
			...
			read();
			if (dwVersion != VmmNative.VMMDLL_MAP_USER_VERSION) {
				throw new VmmException("Bad Version");
			}
	...
	@Structure.FieldOrder({ "dwServiceType", "dwCurrentState", "dwControlsAccepted", "dwWin32ExitCode",
	...
	@Structure.FieldOrder({ "vaObj", "dwOrdinal", "dwStartType", "ServiceStatus", "uszServiceName", "uszDisplayName",
			"uszPath", "uszUserTp", "uszUserAcct", "uszImagePath", "dwPID", "_FutureUse1", "_FutureUse2" })
	...
	class VMMDLL_MAP_SERVICE extends Structure {
		...
		VMMDLL_MAP_SERVICE(Pointer p) {
			...
			read();
			if (dwVersion != VmmNative.VMMDLL_MAP_SERVICE_VERSION) {
				throw new VmmException("Bad Version");
			}
	...
	class VMMDLL_MAP_POOL extends Structure {
		...
		VMMDLL_MAP_POOL(Pointer p) {
			...
			read();
			if (dwVersion != VmmNative.VMMDLL_MAP_POOL_VERSION) {
				throw new VmmException("Bad Version");
			}
	...
	@Structure.FieldOrder({ "vaObject", "dwHandle", "_dwGrantedAccess_iType", "qwHandleCount", "qwPointerCount",
			"vaObjectCreateInfo", "vaSecurityDescriptor", "uszText", "_FutureUse2", "dwPID", "dwPoolTag", "_FutureUse",
			"uszType" })
	...
	class VMMDLL_MAP_HANDLE extends Structure {
		...
		VMMDLL_MAP_HANDLE(Pointer p) {
			...
			read();
			if (dwVersion != VmmNative.VMMDLL_MAP_HANDLE_VERSION) {
				throw new VmmException("Bad Version");
			}
	...
	class VMMDLL_MAP_HEAP extends Structure {
		...
		VMMDLL_MAP_HEAP(Pointer p) {
			...
			read();
			if (dwVersion != VmmNative.VMMDLL_MAP_HEAP_VERSION) {
				throw new VmmException("Bad Version");
			}
	...
	class VMMDLL_MAP_HEAPALLOC extends Structure {
		...
		VMMDLL_MAP_HEAPALLOC(Pointer p) {
			...
			read();
			if (dwVersion != VmmNative.VMMDLL_MAP_HEAPALLOC_VERSION) {
				throw new VmmException("Bad Version");
			}
	...
	boolean VMMDLL_Map_GetHeapAlloc(Pointer hVMM, int dwPID, long qwHeapNumOrAddress,
			PointerByReference ppHeapAllocMap);

	@Structure.FieldOrder({ "vaBase", "cPages", "fPage", "fWow64", "_FutureUse1", "uszText", "_Reserved1",
			"cSoftware" })
	...
	class VMMDLL_MAP_PTE extends Structure {
		...
		VMMDLL_MAP_PTE(Pointer p) {
			...
			read();
			if (dwVersion != VmmNative.VMMDLL_MAP_PTE_VERSION) {
				throw new VmmException("Bad Version");
			}
	...
	@Structure.FieldOrder({ "dwTID", "dwPID", "dwExitStatus", "bState", "bRunning", "bPriority", "bBasePriority",
			"vaETHREAD", "vaTeb", "ftCreateTime", "ftExitTime", "vaStartAddress", "vaStackBaseUser", "vaStackLimitUser",
			"vaStackBaseKernel", "vaStackLimitKernel", "vaTrapFrame", "vaRIP", "vaRSP", "qwAffinity", "dwUserTime",
			"dwKernelTime", "bSuspendCount", "bWaitReason", "_FutureUse1", "_FutureUse2", "vaImpersonationToken",
			"vaWin32StartAddress" })
	...
	class VMMDLL_MAP_THREAD extends Structure {
		...
		VMMDLL_MAP_THREAD(Pointer p) {
			...
			read();
			if (dwVersion != VmmNative.VMMDLL_MAP_THREAD_VERSION) {
				throw new VmmException("Bad Version");
			}
	...
	@Structure.FieldOrder({ "vaBase", "cbImageSize", "fWow64", "uszText", "_FutureUse1", "dwCheckSum",
			"dwTimeDateStamp", "ftUnload" })
	...
	class VMMDLL_MAP_UNLOADEDMODULE extends Structure {
		...
		VMMDLL_MAP_UNLOADEDMODULE(Pointer p) {
			...
			read();
			if (dwVersion != VmmNative.VMMDLL_MAP_UNLOADEDMODULE_VERSION) {
				throw new VmmException("Bad Version");
			}
	...
	@Structure.FieldOrder({ "vaStart", "vaEnd", "vaVad", "dw0", "dw1", "dwu2", "cbPrototypePte", "vaPrototypePte",
			"vaSubsection", "uszText", "_FutureUse1", "_Reserved1", "vaFileObject", "cVadExPages", "cVadExPagesBase",
			"_Reserved2" })
	...
	class VMMDLL_MAP_VAD extends Structure {
		...
		VMMDLL_MAP_VAD(Pointer p) {
			...
			read();
			if (dwVersion != VmmNative.VMMDLL_MAP_VAD_VERSION) {
				throw new VmmException("Bad Version");
			}
	...
	@Structure.FieldOrder({ "tp", "iPML", "pteFlags", "_Reserved2", "va", "pa", "pte", "_Reserved1", "proto_tp",
			"proto_pa", "proto_pte", "vaVadBase" })
	...
	class VMMDLL_MAP_VADEX extends Structure {
		...
		VMMDLL_MAP_VADEX(Pointer p) {
			...
			read();
			if (dwVersion != VmmNative.VMMDLL_MAP_VADEX_VERSION) {
				throw new VmmException("Bad Version");
			}
	...
	@Structure.FieldOrder({ "magic", "wVersion", "wSize", "tpMemoryModel", "tpSystem", "fUserOnly", "dwPID", "dwPPID",
			"dwState", "szName", "szNameLong", "paDTB", "paDTB_UserOpt", "vaEPROCESS", "vaPEB", "_Reserved1", "fWow64",
			"vaPEB32", "dwSessionId", "qwLUID", "szSID", "IntegrityLevel" })
	...
	boolean VMMDLL_ProcessGetInformation(Pointer hVMM, int dwPID, VMMDLL_PROCESS_INFORMATION pProcessInformation,
			LongByReference pcbProcessInformation);
	...
	@Structure.FieldOrder({ "uszCompanyName", "uszFileDescription", "uszFileVersion", "uszInternalName",
			"uszLegalCopyright", "uszOriginalFilename", "uszProductName", "uszProductVersion" })
	...
	@Structure.FieldOrder({ "vaBase", "vaEntry", "cbImageSize", "fWoW64", "uszText", "_Reserved3", "_Reserved4",
			"uszFullName", "tp", "cbFileSizeRaw", "cSection", "cEAT", "cIAT", "_Reserved2", "_Reserved1",
			"pExDebugInfo", "pExVersionInfo" })
	...
	class VMMDLL_MAP_MODULE extends Structure {
		...
		VMMDLL_MAP_MODULE(Pointer p) {
			...
			read();
			if (dwVersion != VmmNative.VMMDLL_MAP_MODULE_VERSION) {
				throw new VmmException("Bad Version");
			}
	...
	boolean VMMDLL_Map_GetModuleFromNameU(Pointer hVMM, int dwPID, String uszModuleName,
			PointerByReference ppModuleMapEntry, int flags);
	...
	@Structure.FieldOrder({ "dwVersion", "dwOrdinalBase", "cNumberOfNames", "cNumberOfFunctions",
			"cNumberOfForwardedFunctions", "_Reserved1", "vaModuleBase", "vaAddressOfFunctions", "vaAddressOfNames",
			"pbMultiText", "cbMultiText", "cMap", "pMap" })
	class VMMDLL_MAP_EAT extends Structure {
		...
		VMMDLL_MAP_EAT(Pointer p) {
			...
			read();
			if (dwVersion != VmmNative.VMMDLL_MAP_EAT_VERSION) {
				throw new VmmException("Bad Version");
			}
	...
	@Structure.FieldOrder({ "vaFunction", "uszFunction", "_FutureUse1", "_FutureUse2", "uszModule", "f32", "wHint",
			"_Reserved1", "rvaFirstThunk", "rvaOriginalFirstThunk", "rvaNameModule", "rvaNameFunction" })
	...
	class VMMDLL_MAP_IAT extends Structure {
		...
		VMMDLL_MAP_IAT(Pointer p) {
			...
			read();
			if (dwVersion != VmmNative.VMMDLL_MAP_IAT_VERSION) {
				throw new VmmException("Bad Version");
			}
	...
	@Structure.FieldOrder({ "name", "MiscVirtualSize", "VirtualAddress", "SizeOfRawData", "PointerToRawData",
	...
	boolean VMMDLL_ProcessGetSectionsU(Pointer hVMM, int dwPID, String uszModule, IMAGE_SECTION_HEADER[] pData,
			int cData, IntByReference pcData);

	boolean VMMDLL_PdbLoad(Pointer hVMM, int dwPID, long vaModuleBase, byte[] szModuleName);

	boolean VMMDLL_PdbSymbolName(Pointer hVMM, String szModule, long cbSymbolAddressOrOffset, byte[] szModuleName,
			IntByReference pdwSymbolDisplacement);
	...
	boolean VMMDLL_PdbTypeChildOffset(Pointer hVMM, String szModule, String uszTypeName, String uszTypeChildName,
			IntByReference pdwSymbolDisplacement);

	@Structure.FieldOrder({ "magic", "wVersion", "wSize", "_FutureReserved1", "vaCMHIVE", "vaHBASE_BLOCK", "cbLength",
			"uszName", "uszNameShort", "uszHiveRootPath", "_FutureReserved" })
	...
	boolean VMMDLL_WinReg_HiveList(Pointer hVMM, VMMDLL_REGISTRY_HIVE_INFORMATION[] pHives, int cHives,
			IntByReference pcHives);

	boolean VMMDLL_WinReg_HiveReadEx(Pointer hVMM, long vaCMHive, int ra, Pointer ptr, int cb,
			IntByReference pcbReadOpt, long flags);

	boolean VMMDLL_WinReg_HiveWrite(Pointer hVMM, long vaCMHive, int ra, byte[] pb, int cb);

	boolean VMMDLL_WinReg_EnumKeyExU(Pointer hVMM, String uszFullPathKey, int dwIndex, byte[] lpName,
			IntByReference lpcchName, LongByReference lpftLastWriteTime);

	boolean VMMDLL_WinReg_EnumValueU(Pointer hVMM, String uszFullPathKey, int dwIndex, byte[] lpValueName,
			IntByReference lpcchValueName, IntByReference lpType, byte[] lpData, IntByReference lpcbData);

	boolean VMMDLL_WinReg_QueryValueExU(Pointer hVMM, String uszFullPathKeyValue, IntByReference lpType, byte[] lpData,
