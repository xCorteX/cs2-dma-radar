...
public class LeechCoreImpl implements ILeechCore {
    ...
    private LeechCoreImpl(String lcNativeLibraryPath, String strDevice, String strRemote, int flagsVerbose,
            long paMax) {
    ...
    public static ILeechCore Initialize(String lcNativeLibraryPath, String strDevice, String strRemote,
    ...
    public byte[] command(long fCommand, byte[] data) {
        ...
        if (0 == Pointer.nativeValue(pbDataOut)) {
            return null;
        }
    ...
    public LeechCoreBar[] getBarInfo() {
        IntByReference pcbDataOut = new IntByReference();
        PointerByReference ppbDataOut = new PointerByReference();
        boolean f = LeechCoreNative.INSTANCE.LcCommand(hLC, ILeechCore.LC_CMD_FPGA_BAR_INFO, 0, null, ppbDataOut,
                pcbDataOut);
        if (!f) {
            throw new LeechCoreException("LeechCore.LcCommand(): failed.");
        }
...
class LeechCoreTlpContextImpl implements ILeechCoreTlpContext {
    ...
    static LeechCoreTlpContextImpl initializeLeechCoreTlpContextImpl(LeechCoreImpl lc, ILeechCoreTlpCallback cbUser) {
        ...
        if (!LeechCoreNative.INSTANCE.LcCommand(lc.hLC, ILeechCore.LC_CMD_FPGA_TLP_CONTEXT, 0, ctx.keyPointer, null,
                null)) {
            ctx.close();
            return null;
        }
        if (!LeechCoreNativeEx.INSTANCE.LcCommand(lc.hLC, ILeechCore.LC_CMD_FPGA_TLP_FUNCTION_CALLBACK, 0, ctx.cbNative,
                null, null)) {
            ctx.close();
            return null;
        }
...
class LeechCoreBarReplyImpl implements ILeechCoreBarReply {
    ...
    public void reply(byte[] data) {
        ...
        if (n.cbData != data.length) {
            throw new LeechCoreException(
                    "LeechCore.replyBarRead(): data length of reply mis-matches requested data length.");
        }
        n.fReadReply = true;
...
class LeechCoreBarContextImpl implements ILeechCoreBarContext {
    ...
    static LeechCoreBarContextImpl initializeLeechCoreBarContextImpl(LeechCoreImpl lc, ILeechCoreBarCallback cbUser) {
        ...
        ctx.key = JnaObjectMap.getInstance().put(ctx);
        ctx.keyPointer = new Pointer(ctx.key.longValue());
        if (!LeechCoreNative.INSTANCE.LcCommand(lc.hLC, ILeechCore.LC_CMD_FPGA_BAR_CONTEXT, 0, ctx.keyPointer, null,
                null)) {
            ctx.close();
            return null;
        }
        if (!LeechCoreNativeEx.INSTANCE.LcCommand(lc.hLC, ILeechCore.LC_CMD_FPGA_BAR_FUNCTION_CALLBACK, 0, ctx.cbNative,
                null, null)) {
            ctx.close();
            return null;
        }
