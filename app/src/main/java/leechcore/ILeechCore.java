 ...
 * LeechCore for Java requires JNA - https://github.com/java-native-access/jna
 * which must be on the classpath.<br>
 * Check out the example code to get started!
 * https://github.com/ufrisk/LeechCore/<br>
 * 
...
public interface ILeechCore {
	 ...
	 * 
	 * @param lcNativeLibraryPath path to vmm.dll / vmm.so native binaries, ex:
	 ...
	 * @return
	 */
	public static ILeechCore initializeLeechCore(String lcNativeLibraryPath, String strDevice, String strRemote,
			int flagsVerbose, long paMax) {
	...
	public static final long LC_OPT_CORE_STATISTICS_CALL_COUNT = 0x4000000900000000L; // R [lo-dword:
																						// LC_STATISTICS_ID_*]
	...
	public static final long LC_OPT_FPGA_ALGO_TINY = 0x0300008400000000L; // RW - 1/0 use tiny 128-byte/tlp read
																			// algorithm.
	public static final long LC_OPT_FPGA_ALGO_SYNCHRONOUS = 0x0300008500000000L; // RW - 1/0 use synchronous (old) read
																					// algorithm.
	public static final long LC_OPT_FPGA_CFGSPACE_XILINX = 0x0300008600000000L; // RW - [lo-dword: register address in
																				// bytes] [bytes: 0-3: data, 4-7:
																				// byte_enable(if wr/set); top bit =
																				// cfg_mgmt_wr_rw1c_as_rw]
	public static final long LC_OPT_FPGA_TLP_READ_CB_WITHINFO = 0x0300009000000000L; // RW - 1/0 call TLP read callback
																						// with additional string info
																						// in szInfo
	public static final long LC_OPT_FPGA_TLP_READ_CB_FILTERCPL = 0x0300009100000000L; // RW - 1/0 call TLP read callback
																						// with memory read completions
																						// from read calls filtered

	/**
	 * Get a device specific option value. Please see defines LC_OPT_* for
	 * information
	...
	/**
	 * Set a device specific option value. Please see defines LC_OPT_* for
	 * information
	 * about valid option values. Please note that option values may overlap between
	 * different device types with different meanings.
	 * 
	...
	public static final long LC_CMD_FPGA_CFGREGCFG_MARKWR = 0x0000010700000000L; // W - write with mask [lo-dword:
																					// register address] [bytes: 0-1:
																					// data, 2-3: mask]
	public static final long LC_CMD_FPGA_CFGREGPCIE_MARKWR = 0x0000010800000000L; // W - write with mask [lo-dword:
																					// register address] [bytes: 0-1:
																					// data, 2-3: mask]
	...
	public static final long LC_CMD_FPGA_CFGSPACE_SHADOW_WR = 0x0000010d00000000L; // W - [lo-dword: config space write
																					// base address]
	...
	public static final long LC_CMD_FPGA_TLP_TOSTRING = 0x0000011200000000L; // RW - convert single TLP to LPSTR;
																				// *pcbDataOut includes NULL terminator.

	public static final long LC_CMD_FPGA_TLP_CONTEXT = 0x2000011400000000L; // W - set/unset TLP user-defined context to
																			// be passed to callback function. (pbDataIn
																			// == LPVOID user context). [not remote].
	public static final long LC_CMD_FPGA_TLP_CONTEXT_RD = 0x2000011b00000000L; // R - get TLP user-defined context to be
																				// passed to callback function. [not
																				// remote].
	public static final long LC_CMD_FPGA_TLP_FUNCTION_CALLBACK = 0x2000011500000000L; // W - set/unset TLP callback
																						// function (pbDataIn ==
																						// PLC_TLP_CALLBACK). [not
																						// remote].
	public static final long LC_CMD_FPGA_TLP_FUNCTION_CALLBACK_RD = 0x2000011c00000000L; // R - get TLP callback
																							// function. [not remote].
	public static final long LC_CMD_FPGA_BAR_CONTEXT = 0x2000012000000000L; // W - set/unset BAR user-defined context to
																			// be passed to callback function. (pbDataIn
																			// == LPVOID user context). [not remote].
	public static final long LC_CMD_FPGA_BAR_CONTEXT_RD = 0x2000012100000000L; // R - get BAR user-defined context to be
																				// passed to callback function. [not
																				// remote].
	public static final long LC_CMD_FPGA_BAR_FUNCTION_CALLBACK = 0x2000012200000000L; // W - set/unset BAR callback
																						// function (pbDataIn ==
																						// PLC_BAR_CALLBACK). [not
																						// remote].
	public static final long LC_CMD_FPGA_BAR_FUNCTION_CALLBACK_RD = 0x2000012300000000L; // R - get BAR callback
																							// function. [not remote].
	public static final long LC_CMD_FPGA_BAR_INFO = 0x0000012400000000L; // R - get BAR info (pbDataOut ==
																			// LC_BAR_INFO[6]).
	...
	/**
	 * Set/Activate a BAR callback. When a BAR access is requested by the host
	 * system
	 * this callback will be called. Only one callback may be active at a given
	 * time.
	...
	/**
	 * Set/Activate a TLP callback. When a PCIe TLP packet is received this callback
	 * will be called.
